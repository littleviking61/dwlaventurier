...
<?php
	...
	function upload_sample_image() {
		$url = get_template_directory_uri().'/dummy-content/demo-image.png';
		...
		$image_query = new WP_Query( array( 'post_type' => 'attachment', 'posts_per_page' => 1, 'post_status' => 'inherit', ) );
		while( $image_query->have_posts() ) {
			$image_query->the_post();
			$post_id = get_the_ID();
		}
		$array = wp_generate_attachment_metadata( $post_id, $upload['file'] );
		// print_r($array); die();
	...
	function process_attachment( $post, $url ) {
		...
		if( $url != '' ) {
			...
			$upload['url'] = $upload_dir['baseurl'].'/2013/01/demo-image.png';
			$upload['file'] = $upload_dir['basedir'].'/2013/01/demo-image.png';
			$this->url_remap[$url] = $upload['url'];
			$this->url_remap[$post['guid']] = $upload['url']; // r13735, really needed?
		}
		...
		$image_arr = array( 
			'width' => 800,
		 	'height' => 800,
		 	'file' => '2013/01/demo-image.png',
		 	);

		foreach( $this->image_size as $key => $value ) {
			$image_arr['size'][$key] = array( 
		 			'file' => 'demo-image-'.$value['width'].'x'.$value['height'].'.png',
		 			'width' => $value['width'], 
		 			'height' => $value['height'], 
		 			'mime-type' => 'image/gif',
		 			 );
